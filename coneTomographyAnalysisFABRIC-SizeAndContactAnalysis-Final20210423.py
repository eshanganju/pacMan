...
for scan in scanList:
    ...
    if True:
       	...
       	if scan == 1: currentSubregion = int(7)
        if scan == 5: currentSubregion = int(1)
        ...
        binMapLoc = outputLoc + currentSampleName + '-binMap.tif'
        binMap = tf.imread(binMapLoc)
        binMap = binMap//255	# this is done to make the binary map with 1s and 0s only
