...
r.plotGLI(four.filteredGreyLevelMap)
...
four.labelledMap = tiffy.imread( 'watershedSegmentation-edited-edited.tif' ) # After manual editing
...
five.labelledMap = tiffy.imread( 'watershedSegmentation-edited-edited.tif' ) # After manual editing
...
# Create Aggregates
six = Aggregate.Aggregate( 'six', srSixD50, calib, 16 )

del srSixD50
gc.collect()

# Filteration
f.filterDenoiseNlm( six )
...
s.removespecks( six )
six.binaryMap[ 0, :,: ] = 0
six.binaryMap[ six.binaryMap.shape[ 0 ] - 1, :, : ] = 0

# Euclid-Marker-Topo
s.euclidDistanceMap( six )
s.localhMaxima( six, 4 )
s.topoWatershed( six )
six.labelledMap = tiffy.imread( 'watershedSegmentation-edited-edited.tif' ) # After manual editing
s.resetParticleList( six )

# Analysis
m.measureParticleSizeDistribution( six )
