...
def getParticleSizeArray( labelledMapForParticleSizeAnalysis, calibrationFactor=1,
	...
	particleSizeDataSummary = np.zeros( ( numberOfParticles + 1 , 11 ) )  # Giving issues with axis having size of 995
	#particleSizeDataSummary[0,:] = ['Label index', 'Volume (um^3)', 'Equivalent Diameter (um)', 'Surface Area (um^2)', 'Sphericity', 'Centroidal Max (um)',\
	#'Centroidal Med (um)', 'Centroidal Min (um)', 'Convex volume (um^3)', 'Concave volume (um^3)', 'Overlapped Volume (um^3)']
	...
	for particleNum in range( 1, numberOfParticles + 1 ):
		...
		# Surface area
		surfA = computeSurfaceAreaOfLabel( labMap=labelledMapForParticleSizeAnalysis, label=int(particleNum) )
		...
		croppedParticle, convex, concave, overlap, convexVol, concaveVol, overlapVol = calculateEllipsoidOverUnder( labelMap=labelledMapForParticleSizeAnalysis,label=int(particleNum) )
...
def calculateEllipsoidOverUnder(labelMap, label):
	...
	for n in np.arange(0,rotCentPointCloud.shape[0]-1):
		croppedParticle[(math.floor( rotCentPointCloud[n,0] )+originZ,math.floor( rotCentPointCloud[n,1] )+originY,math.floor( rotCentPointCloud[n,2] )+originX)]=3
		croppedParticle[(math.ceil( rotCentPointCloud[n,0] )+originZ,math.ceil( rotCentPointCloud[n,1] )+originY,math.ceil( rotCentPointCloud[n,2] )+originX)]=3
		
	
	#Generate ellipse with a,b,c dimensions defined by z,y,x axes
	ellipsoid = createVoxelizedEllipsoid( particle = croppedParticle, a = cloudZ/2, b = cloudY/2, c = cloudX/2 ) 
...
def createVoxelizedEllipsoid( particle, a, b, c ):
	...
	The value of the ellipse voxels are 1.
	...
	for X in xRange:
		...
		for Y in yRange:
			...
			Zhi = originZ+int(Z)+1
			Zlo = originZ-int(Z)
			ellipsoid[Zlo:Zhi,int(originY+Y),int(originX+X)]=1
		
	return ellipsoid
	
def getLargestInscribedSphere(labMap, label):
	"""Use max EDM for each particle
	"""
