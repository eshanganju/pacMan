...
for edgeLength in range( edgeLengthMin , edgeLengthMax ):
    ...
    while gsdOK == False:
        ...
        Plot.grainSizeDistribution(gsd1,gsd2,gsd3,gsd4,originalGSD)

        exitLoop = input('Is any gsd ok(y/[n])?')
        ...
        else: print('Use user threshold to update binary map')

    Br = Measure.relativeBreakage(originalGSD,gsdForBr)

    contactTableRW, contactTableITK = Measure.contactNormalsSpam()
    ...
    corLabName = outputFolderLocation + 'corLabMap.tiff'
    neCorLabName = outputFolderLocation + 'noEdgeCorLabMap.tiff'

    tf.imsave(gliName,gliMap.astype('uint32'))
    tf.imsave(labName,labMap.astype('uint32'))
    tf.imsave(correctedLabName,corLabMap.astype('uint32'))
    tf.imsave(noEdgeCorrectedLabName,neCorLabMap.astype('uint32'))
